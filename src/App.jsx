...
const userFirebaseConfig = {
  ...
  storageBucket: "redmarkfiles.appspot.com",
  messagingSenderId: "94920695515",
  appId: "1:94920695515:web:8142bf6289964ca381b6fe",
  measurementId: "G-BWG56DW2KZ"
};
...
// --- Task Button Component (Moved outside App) ---
const SmallTaskButton = ({ onClick, disabled, isLoading, title, token, icon, gradient, isSdkReady }) => (
    <button
        onClick={onClick}
    ...
    >
        ...
        <span className="z-10 text-sm font-semibold">{title}</span>
        <span className="z-10 text-xs font-bold bg-white/25 px-2 py-0.5 rounded-full mt-1"> +{token} </span>
...
const BottomNavItem = ({ label, icon, isActive, onClick }) => (
    <button onClick={onClick} className={`flex flex-col items-center justify-center p-2 rounded-lg transition-all duration-200 group w-full ${isActive ? 'bg-gradient-to-br from-purple-600 to-indigo-600 text-white shadow-md scale-100' : 'text-slate-400 hover:text-white hover:bg-slate-700/50 active:bg-slate-700'}`} aria-label={label} role="tab" aria-selected={isActive}>
        {icon}
        <span className={`text-[11px] font-medium mt-1 ${isActive ? 'opacity-100' : 'opacity-90 group-hover:opacity-100'}`}>{label}</span> 
    </button>
);
BottomNavItem.propTypes = { label: PropTypes.string.isRequired, icon: PropTypes.node.isRequired, isActive: PropTypes.bool.isRequired, onClick: PropTypes.func.isRequired };

// --- Premium Feed Component ---
function PremiumFeed({ videos, userProfile, db, userId, appId, showMessage, setIsLoading, isLoading, setCurrentTab }) { 
    ...
    const handleUnlock = useCallback(async (video) => { 
        ...
        try {
            const userProfileRef = doc(db, `artifacts/${appId}/users/${userId}/profile`, "data");
            const currentUnlocked = Array.isArray(userProfile?.unlockedVideos) ? userProfile.unlockedVideos : [];
